...
def test():
    ...
    for index, block in enumerate(blocks):
        ...
        set_matrix_requests.append(post(url, data, endpoint='/matrix', parallel=True))
    set_matrix_responses = grequests.map(set_matrix_requests)
    print('Broadcast time: ' + str(time.time() - start_time_broadcast))
    print(set_matrix_responses)
    ...
    for index, _ in enumerate(blocks):
        url = urls[index]
        requests_list.append(get(url, endpoint='/solve'))
